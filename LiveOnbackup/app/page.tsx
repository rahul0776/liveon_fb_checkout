...
async function getUser() {
  ...
  const cachedUser = cookieStore.get("fb_user")?.value;

  if (!token) {
    return null;
  }

  // If we have a cached user, use it! This saves a Graph API call and avoids rate limits.
  if (cachedUser) {
    try {
      return JSON.parse(cachedUser);
    } catch (e) {
      console.error("DEBUG: Failed to parse cached user cookie");
    }
  }

  // Fallback: Fetch from Facebook only if cache is missing
  try {
    const res = await fetch(`https://graph.facebook.com/v18.0/me?access_token=${token}&fields=id,name`, {
      cache: "no-store"
    });
    ...
    if (res.ok) {
      ...
      cookieStore.set("fb_user", JSON.stringify(data), {
        httpOnly: true,
    ...
    console.error("DEBUG: getUser fallback fetch error:", e);
  }
  return null;
}
...
export default async function Home() {
  ...
  console.log("DEBUG: Rendering Home page, user is:", user ? user.name : "Anonymous");
  ...
  return (
    <div className="space-y-24 overflow-x-hidden">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    {user ? (
                      <div className="flex gap-4 items-center">
                        <span className="text-white">Welcome back, {user.name}</span>
                        <Button href="#dashboard" rightIcon={<ArrowRight className="h-4 w-4" />}>
                          Go to Dashboard
                        </Button>
                      </div>
                    ) : (
      ...
      <section id="dashboard" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-0">
        {user ? (
          ...
          <>
            ...
            <div className="mt-10 grid gap-6 md:grid-cols-3">
              {[{
                ...
                copy: "We authenticate via Facebook OAuth, using state tokens to keep every session secure.",
              },
              {
                step: "02",
                title: "Collect & curate",
                copy: "Posts, photos, and reactions are sorted into chapters that you can explore with filters and timelines.",
              },
              {
                step: "03",
                title: "Download in minutes",
                copy: "Checkout unlocks polished ZIP archives from Azure Blob Storage with expiring SAS links.",
              }].map((item, index) => (
