...
# -------------------------------------------------------------------
# Post-payment bypass: when a user just paid for the scrapbook and got
# redirected from FbMemories, Streamlit Cloud may have handed them a
# fresh container with no cached session. Rather than wall them off at
# the login gate, render a minimal "Your scrapbook is ready" view with
# a direct download link. The SAS URL doesn't need fb_token — only
# Azure Blob access.
# -------------------------------------------------------------------
_pp_backup = (
    st.session_state.get("selected_backup")
    or qp_get("backup")
    or qp_get("blob_folder")
)
_pp_flag = bool(st.session_state.get("_post_payment_flow")) or bool(qp_get("scrapbook_just_paid"))

if _pp_backup and _pp_flag and not st.session_state.get("fb_token"):
    # Verify this is a real paid scrapbook with a built PDF before showing download
    try:
        _paid_ok = container_client.get_blob_client(f"{_pp_backup}/.paid.scrapbook").exists()
        _pdf_ok = container_client.get_blob_client(f"{_pp_backup}/scrapbook.pdf").exists()
    except Exception:
        _paid_ok = False
        _pdf_ok = False

    if _paid_ok and _pdf_ok:
        # Build a SAS URL that forces the PDF to download (not open in browser)
        _dl_url = None
        try:
            import re as _re
            ...
            if _m and _k:
                ...
                _filename = "scrapbook.pdf"
                _sas_token = generate_blob_sas(
                    account_name=_m.group(1),
                    container_name="backup",
                    blob_name=f"{_pp_backup}/scrapbook.pdf",
                    account_key=_k.group(1),
                    permission=BlobSasPermissions(read=True),
                    expiry=_dt.utcnow() + _td(minutes=60),
                    content_disposition=f'attachment; filename="{_filename}"',
                )
                _dl_url = f"https://{_m.group(1)}.blob.core.windows.net/backup/{_pp_backup}/scrapbook.pdf?{_sas_token}"
        except Exception:
            pass

        st.markdown("""
        <div style="text-align:center; padding:40px 20px;">
          <div style="font-size:14px; color:#888; letter-spacing:4px; margin-bottom:8px;">✦ YOUR STORY ✦</div>
          <h1 style="font-weight:800; margin:0 0 12px 0;">Facebook <span style="color:#F6C35D; font-style:italic;">Memories</span></h1>
          <p style="color:#aaa; max-width:600px; margin:0 auto 28px;">Your payment is confirmed and your scrapbook PDF is ready.</p>
        </div>
        ...
        with col_c:
            if _dl_url:
                # Auto-trigger download on page load via a hidden anchor + JS
                    ...
                    <script>
                      (function() {{
                        try {{
                          ...
                          a.href = {_json.dumps(_dl_url)};
                          a.download = "scrapbook.pdf";
                ...
                st.link_button("📖 Download Scrapbook PDF", _dl_url,
                               use_container_width=True, type="primary")
                st.caption("Your download should start automatically. If not, click the button above.")
            else:
                st.warning("Couldn't generate download link. Please log in again to access your scrapbook.")
        st.stop()
